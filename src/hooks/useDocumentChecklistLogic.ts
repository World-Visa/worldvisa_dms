...
export function useDocumentChecklistLogic({
  ...
  const [addingDocumentId, setAddingDocumentId] = useState<string | undefined>(
    undefined,
  );
  const [isDocumentAdded, setIsDocumentAdded] = useState(false);
  const [addedDocumentId, setAddedDocumentId] = useState<string | undefined>(
    undefined,
  );
  ...
  const allDocumentTypes = useMemo(() => {
    if (isClientView && checklistData?.data) {
      const checklistDocuments = checklistData.data.map(
        (item: { document_category: string; document_type: string }) => {
          ...
          if (item.document_category === "Identity") {
            ...
            if (documents && documents.length > 0) {
              const companyDoc = documents.find(
                (doc) =>
                  doc.document_category &&
                  doc.document_category.includes("Company Documents") &&
                  doc.company_name ===
                    (item as unknown as ChecklistItem).company_name,
              );
  ...
  const matchesCategory = useCallback(
    (itemCategory: string, targetCategory: string) => {
      const categoryLabel = mapCategoryLabel(itemCategory);

      if (targetCategory === "company") {
        return (
          categoryLabel === "Company Documents" || categoryLabel === "Company"
        );
      }
  ...
  const handleAddToPendingChanges = useCallback(
    async (document: ChecklistDocument) => {
