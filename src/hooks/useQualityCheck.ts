    ...
    onSuccess: (response) => {
      try {
        ...
        if (response.success === true) {
          toast.success(
            response.message ||
              "Application pushed for quality check successfully!",
          );
          ...
          const errorMessage =
            response.message || "Failed to push for quality check";
        ...
        toast.error(
          "An unexpected error occurred while processing the quality check response",
        );
