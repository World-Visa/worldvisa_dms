  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Record<string, number>> => {
      ...
      try {
        ...
        const promises = documentIds.map(async (documentId) => {
          try {
            ...
            if (response.status === 'error') {
              console.warn(`Failed to fetch comments for document ${documentId}:`, response.message);
            ...
            console.warn(`Error fetching comments for document ${documentId}:`, error);
            return { documentId, count: 0 };
          }
        });
        ...
        return results.reduce((acc, { documentId, count }) => {
        ...
        return documentIds.reduce((acc, documentId) => {
          acc[documentId] = 0;
          return acc;
        }, {} as Record<string, number>);
      }
