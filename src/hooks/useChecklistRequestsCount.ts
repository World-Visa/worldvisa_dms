...
export function useChecklistRequestsCount(options: UseChecklistRequestsCountOptions = {}) {
  const {
    enabled = true,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const validRequests = response.data.filter(req => req.id && req.id.trim() !== '' && req.Checklist_Requested === true);
...
export type ChecklistRequestsCountHook = ReturnType<typeof useChecklistRequestsCount>;
