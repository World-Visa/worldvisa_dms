...
export function usePerformanceMonitor(componentName: string, enabled: boolean = true) {
  ...
  return {
    renderCount: renderCount.current,
    measureAsync: useCallback(async <T>(operation: () => Promise<T>, operationName: string): Promise<T> => {
...
export function useComponentLifecycle(componentName: string, enabled: boolean = true) {
  ...
  return {
    ...
    getLifecycleTime: () => isMounted.current ? performance.now() - mountTime.current : 0
  };
}
