...
import {
  createReviewRequest,
  createMultipleReviewRequests,
  ReviewRequestData,
} from "@/lib/api/reviewRequest";
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        const successfulReviewRequests: Array<{
          _id: string;
          document_id: string;
        }> = [];
        ...
        results.forEach((result, resultIndex) => {
          if (result.success && result.data) {
            ...
            const reviewRequests = Array.isArray(result.data)
              ? result.data
              : [result.data];
            ...
            reviewRequests.forEach((reviewRequest) => {
              if (reviewRequest && reviewRequest._id) {
                ...
                const documentId =
                  originalRequest?.documentId || reviewRequest.document_id;
                ...
                if (reviewRequest._id.startsWith("temp-")) {
                  console.log(
                    "Skipping message send for temporary review request ID:",
                    reviewRequest._id,
                  );
                  return;
                }
        ...
        const failedMessages = messageResults.filter(
        ...
        if (responseTime > 5000) {
          console.warn(
            `Slow review request batch: ${responseTime}ms for ${requests.length} requests`,
          );
        }
    ...
    onMutate: async ({ requestedTo }) => {
      ...
      toast.loading(
        `Sending review requests to ${requestedTo.length} admin${requestedTo.length !== 1 ? "s" : ""}...`,
        {
 ...
 */
export function useSingleReviewRequest({
  onSuccess,
  onError,
}: UseReviewRequestProps = {}) {
  const queryClient = useQueryClient();
