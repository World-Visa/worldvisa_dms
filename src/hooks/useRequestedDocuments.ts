...
function sortByRequestedAtDesc(documents: RequestedDocument[]): RequestedDocument[] {
...
export function useAllRequestedDocumentsPaginated(
  ...
  return useQuery({
    ...
    enabled: options?.enabled !== undefined ? options.enabled : (!!user && user.role === 'master_admin'),
    placeholderData: keepPreviousData,
    select: (data) => {
