...
export function useUploadSampleDocument() {
  ...
  return useMutation({
    mutationFn: (data: UploadSampleDocumentRequest) =>
      uploadSampleDocument(data),
...
export function useUpdateSampleDocument() {
  ...
  return useMutation({
    mutationFn: (data: UpdateSampleDocumentRequest) =>
      updateSampleDocument(data),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: queryKey(variables.applicationId),
      });
...
export function useSampleDocument({
  ...
  return useQuery({
    ...
    queryFn: () =>
      sampleDocumentService.getSampleDocument(documentType, category),
...
export function useHasSampleDocument({
  ...
  return useQuery({
    ...
    queryFn: () =>
      sampleDocumentService.hasSampleDocument(documentType, category),
    enabled: enabled && !!documentType && !!category,
    staleTime: 5 * 60 * 1000, // 5 minutes
    gcTime: 10 * 60 * 1000, // 10 minutes
    retry: 2,
  ...
  return {
    downloadSample: async (samplePath: string, fileName: string) => {
      try {
        ...
        await sampleDocumentService.downloadSampleDocument(
          samplePath,
          fileName,
        );
        return { success: true };
 ...
 */
export function useSampleDocumentsMetadata(
  documentTypes: Array<{ documentType: string; category: string }>,
) {
  return useQuery({
    ...
    queryFn: () => {
      return documentTypes.map(({ documentType, category }) => ({
        documentType,
        category,
        hasSample: sampleDocumentService.hasSampleDocument(
          documentType,
          category,
        ),
        sampleDocument: sampleDocumentService.getSampleDocument(
          documentType,
          category,
        ),
      }));
