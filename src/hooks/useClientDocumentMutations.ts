...
export function useClientUploadDocument() {
  ...
  return useMutation<ClientUploadDocumentResponse, Error, ClientUploadDocumentRequest>({
...
export function useClientReuploadDocument() {
  ...
  return useMutation<ClientReuploadDocumentResponse, Error, ClientReuploadDocumentRequest>({
    mutationFn: clientReuploadDocument,
    onSuccess: (data, variables) => {
      // First, optimistically update the document status in the cache
      queryClient.setQueryData<{ success: boolean; data: ClientDocument[] }>(['client-documents', variables.clientId], (old) => {
