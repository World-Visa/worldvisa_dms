...
export function useSendRequestedDocumentMessage() {
  ...
  return useMutation({
    ...
    onMutate: async ({ documentId, reviewId, data }) => {
      ...
      const previousResponse = queryClient.getQueryData<RequestedDocumentMessagesResponse>(
    ...
    onSuccess: (responseData, variables, context) => {
      ...
      queryClient.setQueryData<RequestedDocumentMessagesResponse>(
        ['requested-document-messages', variables.documentId, variables.reviewId],
        (old) => {
          if (!old) {
            return {
              status: 'success',
          ...
          const filtered = old.data.filter(msg => msg._id !== context?.optimisticMessage._id);
          return {
            ...old,
          ...
          (oldResponse) => {
            ...
            switch (event.type) {
              case 'message_added': {
                ...
                const exists = oldResponse.data.some(msg => msg._id === event.message._id);
                if (!exists) {
                  return {
                    ...oldResponse,
              ...
              case 'message_deleted': {
                return {
                  ...oldResponse,
                  data: oldResponse.data.filter(msg => msg._id !== event.message._id)
                };
              }
            }
