...
const DmsTooltip = React.forwardRef<HTMLDivElement, DmsTooltipProps>(
  ({ active, payload, label, rows, showTotal = true, title, dateFormatter }, ref) => {
    ...
    const tooltipRows: DmsTooltipRow[] =
      rows ||
      payload?.map((item) => ({
        key: item.dataKey,
...
DmsTooltip.displayName = 'DmsTooltip';

export { ChartContainer, ChartTooltip, ChartTooltipContent, ChartLegend, ChartLegendContent, DmsTooltip, ChartStyle };
