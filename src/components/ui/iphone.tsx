...
export interface IphoneProps extends HTMLAttributes<HTMLDivElement> {
  src?: string
  videoSrc?: string
  /** Interactive or custom content in the screen area (e.g. iframe). Takes precedence over src / videoSrc. */
  screenContent?: ReactNode
}

const screenSlotStyle = {
  left: `${LEFT_PCT}%`,
  top: `${TOP_PCT}%`,
  width: `${WIDTH_PCT}%`,
  height: `${HEIGHT_PCT}%`,
...
export function Iphone({
  ...
  const hasMedia = hasVideo || !!src || !!screenContent
  ...
  return (
    ...
    >
      {screenContent ? (
        <div
          className="pointer-events-auto absolute z-0 flex min-h-0 flex-col overflow-hidden"
          style={screenSlotStyle}
        >
          {screenContent}
        </div>
      ) : hasVideo ? (
