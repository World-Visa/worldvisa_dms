...
const TableFooter = React.forwardRef<HTMLTableSectionElement, TableFooterProps>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn('bg-background sticky bottom-0 shadow-[0_0_0_1px_var(--color-neutral-alpha-200)]', className)}
    {...props}
  />
));
...
const TableRow = React.forwardRef<HTMLTableRowElement, TableRowProps>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn('[&>td]:border-neutral-alpha-200 [&>td]:border-b last-of-type:[&>td]:border-0', className)}
    {...props}
  />
));
