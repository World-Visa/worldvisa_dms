...
export function TimelineStepNumber({ index }: { index: number }) {
  return (
    <div className="text-label-xs bg-neutral-50 text-neutral-950 flex h-6 w-6 shrink-0 items-center justify-center rounded-full p-0.5 text-xs font-medium shadow-[0px_0px_0px_1px_#FFF,0px_0px_0px_2px_#E1E4EA]">
      {index + 1}
    </div>
  );
}
...
export function TimelineLine({ variant = 'default' }: { variant?: 'default' | 'continuous' }) {
  ...
  return <div className="absolute left-3 top-6 h-[calc(100%+2rem)] w-px -translate-x-1/2 bg-neutral-200" />;
}
  ...
  return (
    <motion.div {...stepAnimation(index)} className="relative flex min-w-0 gap-6">
      ...
      <div className="min-w-0 flex-1">
        <div className="text-label-sm capitalize text-neutral-950">{title}</div>
