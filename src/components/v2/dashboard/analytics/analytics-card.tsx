...
import { AnimatedNumber } from '@/components/v2/dashboard/animated-number';
  ...
  return (
    <div className={cn('bg-bg-white rounded-xl border-none p-2.5 shadow-box-xs w-full', className)}>
      <div className="flex flex-col gap-1">
          ...
          {percentageChange !== undefined && (
            <div className="analytics-card-trend shrink-0">
              <div className="flex items-center gap-1 px-1">
                ...
                <span className={cn('text-xs uppercase', trendColors.text)}>
        ...
        <div className="text-3xl text-text-strong font-medium">
          <AnimatedNumber value={value} isLoading={isLoading} />
        </div>

        {description && <div className="text-sm text-text-soft">{description}</div>}
      </div>
    </div>
  );
}
