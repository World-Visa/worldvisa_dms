...
export function DocumentsSummary({
  ...
  const requestReviewCount = documents?.filter(
    (doc) => doc.status == "request_review",
  ).length;
  const rejectedReviewCount = documents?.filter(
        ...
        {summaryCards.map((card) => {
          ...
          const cardContent = (
            <CardContent className="p-2 w-full">
              ...
              <span className="text-lg flex justify-center items-center font-bold text-gray-900">
                {card.count}
              </span>
            </CardContent>
          );
