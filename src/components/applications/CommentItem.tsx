            ...
            <div className="flex items-start space-x-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ...
                    {isClientComment ? (
                        ...
                        <User className={`h-4 w-4 ${
                            isImportant ? 'text-red-600' : 
                            isAdminComment ? 'text-green-600' : 'text-gray-600'
                        }`} />
                    )}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                            <span className={`text-sm font-semibold ${
                                isImportant ? 'text-red-900' : 
                                isClientComment ? 'text-blue-900' :
                                isAdminComment ? 'text-green-900' : 'text-gray-900'
                            }`}>
                                {comment.added_by}
                                {isClientComment && (
                    ...
                    <p className={`text-sm leading-relaxed ${
                        isImportant ? 'text-purple-800' : 
                        isClientComment ? 'text-blue-800' :
                        isAdminComment ? 'text-green-800' : 'text-gray-700'
                    }`}>
                        {comment.comment}
                    </p>
                </div>
