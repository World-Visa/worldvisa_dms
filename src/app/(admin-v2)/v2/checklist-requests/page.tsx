...
const ChecklistRequestsPage = memo(function ChecklistRequestsPage() {
  ...
  const { totalRequests, currentRequests } = useMemo(() => {
    ...
    const total =
      data?.pagination?.totalItems ?? (rawRequests.length === 0 ? 0 : valid.length);
    return {
      totalRequests: total,
      currentRequests: valid,
    };
  }, [data?.data, data?.pagination?.totalItems]);
      ...
      <div className="">
        <div className="mt-8 mb-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              <h2 className="text-2xl font-medium text-foreground mb-2 flex items-center gap-2">
                Checklist Requests
              </h2>
            </div>
