...
class SampleDocumentService {
  ...
  getSampleDocument(
    ...
    const documentWithSample = this.findDocumentWithSample(
      documentType,
      category,
    );
  ...
  async downloadSampleDocument(
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Sample document not accessible: ${response.status} ${response.statusText}`,
          );
        }
      } catch (fetchError) {
        // Log but don't fail - the document might still be downloadable
        console.warn(
          "Could not verify sample document accessibility:",
          fetchError,
        );
      }
      ...
      throw new Error(
        `Failed to download sample document: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private findDocumentWithSample(
    documentType: string,
    category: string,
  ): DocumentTypeWithSample | null {
