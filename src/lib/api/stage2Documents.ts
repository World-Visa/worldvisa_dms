...
export async function updateStage2Document(
  ...
  if (
    request.metadata.date !== undefined &&
    request.metadata.date !== null &&
    String(request.metadata.date).trim() !== ""
  ) {
    metadata.date = String(request.metadata.date).trim();
  }
  if (
    request.metadata.deadline !== undefined &&
    request.metadata.deadline !== null &&
    String(request.metadata.deadline).trim() !== ""
  ) {
    metadata.deadline = String(request.metadata.deadline).trim();
  }
  if (
    request.metadata.outcome_date !== undefined &&
    request.metadata.outcome_date !== null &&
    String(request.metadata.outcome_date).trim() !== ""
  ) {
    metadata.outcome_date = String(request.metadata.outcome_date).trim();
  }
  if (
    request.metadata.outcome !== undefined &&
    request.metadata.outcome !== null &&
    String(request.metadata.outcome).trim() !== ""
  ) {
...
export async function reuploadStage2Document(
  ...
  if (!allowedExtensions.some((ext) => fileName.endsWith(ext))) {
    throw new Error(
      `File "${request.file.name}" has an unsupported file type.`,
    );
  }
  if (!allowedMimeTypes.includes(request.file.type)) {
    throw new Error(
      `File "${request.file.name}" has an unsupported MIME type.`,
    );
  }
  ...
  if (request.file.size > 5 * 1024 * 1024) {
    throw new Error(
      `File "${request.file.name}" is too large. Maximum file size is 5MB.`,
    );
  }
