 ...
 */
export async function getChecklistRequests(params: ChecklistRequestsParams = {}): Promise<ChecklistRequestsResponse> {
  try {
    const { page = 1, limit = 20 } = params;

    // Use the centralized API configuration (Admin endpoint)
    const fullUrl = getFullUrl(API_CONFIG.ENDPOINTS.CHECKLIST_REQUESTS.LIST, { page, limit });
    ...
    if (!Array.isArray(response.data)) {
      throw new Error('Invalid data format: expected array of checklist requests');
    }
    ...
    throw new Error('An unexpected error occurred while fetching checklist requests. Please try again.');
  }
}
 ...
 */
export async function getChecklistRequestByLeadId(leadId: string): Promise<ChecklistRequestItem | null> {
  try {
    ...
    throw new Error('An unexpected error occurred while fetching checklist request.');
  }
}
