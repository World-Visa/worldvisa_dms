import { DescriptionValidation, DESCRIPTION_CONSTRAINTS } from '@/types/description';
...
export function sanitizeDescription(description: string): string {
  return description
    .trim()
    .slice(0, DESCRIPTION_CONSTRAINTS.MAX_LENGTH);
}
...
export function isDescriptionLengthValid(description: string): boolean {
  ...
  return trimmed.length >= DESCRIPTION_CONSTRAINTS.MIN_LENGTH && 
