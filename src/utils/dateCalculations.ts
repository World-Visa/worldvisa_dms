export const calculateDuration = (fromDate: string, toDate: string): string => {
  const from = new Date(
    fromDate.includes("-") && fromDate.split("-").length === 2
      ? fromDate + "-01"
      : fromDate,
  );
  const to = new Date(
    toDate.includes("-") && toDate.split("-").length === 2
      ? toDate + "-01"
      : toDate,
  );
...
export const generateCurrentEmploymentDescription = (
  companyName: string,
  fromDate: string,
): string => {
...
export const generatePastEmploymentDescription = (
  companyName: string,
  fromDate: string,
  toDate: string,
): string => {
...
export const generateCompanyDescription = (
